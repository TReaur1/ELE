...
- **PROGRAM 中禁止声明任何变量**：含 FB 实例、定时器（TON/TP）、边沿检测（R_TRIG/F_TRIG）、中间变量。ST 程序文件只写逻辑语句。
- **变量表是唯一变量来源**：所有变量集中定义在 AutoShop CSV 变量表中，ST 引用的每个符号都必须能在变量表中找到，二者一一对应。
- 变量表分类：
  - `io`    —— 物理输入输出（X / Y 映射变量）
  - `host`  —— 上位机通讯（host_Send_* / host_Rcv_*）
  - `hmi`   —— 触摸屏变量
  - `con`   —— 内部控制：标量、结构体、锁存 / 许可 / 边沿标志
  - `inst`  —— 内部实例：FB 实例、定时器 TON/TP、边沿 R_TRIG/F_TRIG
  - `mc`    —— Modbus 主站 / 预留
...
2. **故障锁存**：故障 SET / RESET；复位需"上升沿 + 故障源已消除"确认，不允许故障仍存在时复位成功。
...
8. **输入滤波**：急停不滤波（安全响应最快）；物料检测 / 限位 20ms（`FB_Debounce`）。
9. **通讯联锁**：Modbus TCP 断开 → `CommLost=TRUE` → `RunOK=0` → 自动停止，不依赖上位机主动停机。
...
以桌面 `C:\Users\kaanh\Desktop\汇川表` 内三个规范 CSV 为基准制表，任何 CSV 必须严格按下列表头输出，列顺序不可调换、表头不可改名、末尾保留逗号。

### 8.1 变量表格式（汇川规范变量表）

```
序号,变量名,数据类型,隐藏初始值,初始值,掉电保持,网络公开,注释,软元件地址,
```

- `序号`：从 1 递增。
- `变量名`：仅英文标识符（R1），如 `in_eStop` / `host_Send_Start`。
- `数据类型`：IEC 大写基础类型 / 结构体名 / 库类型。
- `隐藏初始值` / `初始值`：按工程需要填写，默认空。
- `掉电保持` / `网络公开`：布尔标记，可空。
- `注释`：中文注释。
- `软元件地址`：物理地址（如 `X0` / `Y1` / `M0`）或空。
...
- `类别`：仅允许 `IN / OUT / INOUT / VAR`（对应 VAR_INPUT / VAR_OUTPUT / VAR_IN_OUT / 内部变量）。
- `名称`：接口参数用描述式英文命名（如 `PowerEnable` / `Stop` / `Home`），不带类型前缀；FB 内部变量用匈牙利式类型前缀（b/f/d/w/i/s，见"结构与 FB 成员命名"）。
- `数据类型`：完整类型（基础类型 / 库类型 `TRIG.R_TRIG` 等）。
- `注释`：中文注释。
...
- 每行一个结构体成员，`成员变量名` 仅英文标识符（R1）。
- `数据类型`：BOOL / DINT / REAL / STRING 等，或嵌套结构体。
- `注释`：中文注释；结构体定义处必须列出全部成员并逐行注释（R3）。
- 示例：`1,member_1,BOOL,,`

### 8.4 结构体数组成员格式（强制）

- 结构体数组变量**仅支持一维与二维两种格式**，以 BOOL 为例：一维 `BOOL[1]`、二维 `BOOL[1,1]`。
- 一维数组：数据类型 `T[n]`（如 `BOOL[1]`）；二维数组：数据类型 `T[n,m]`（如 `BOOL[1,1]`）。
- 结构体表中**数组成员必须占两行**：
  - 第一行：`序号,数组名,T[n],,`（一维）或 `序号,数组名,"T[n,m]",,`（二维，`T[n,m]` 含逗号，CSV 中须加双引号），注释填数组用途。
  - 第二行（序号留空）：`数组名[0],T,,`（一维）或 `数组名[0,0],T,,`（二维），`数据类型` 为去掉数组符号后的基础类型（如 `BOOL`）。
- 示例（一维/二维）：
  ```
  12,test,BOOL[1],,
  ,test[0],BOOL,,
