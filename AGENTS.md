...
- **LiteST 位运算字面量禁止十进制**（2026-08-27）：`OR / AND / XOR` 用于整型位运算时，操作数字面量仅支持非十进制格式（`2#0001` / `8#` / `16#`），十进制裸数字（如 `x OR 1`）不合规；位编码一律写二进制 `2#`（bit 位置直观，如 `OR 2#0100`）。逻辑运算（BOOL 型 `OR bFlag`）不受限。
...
**四模块架构（2026-08-27 定稿）**：`io 物理层 → con 控制层 → host 通讯层 → status 状态层`，各层只读写全局变量表，互不直接调用。

| 程序块 | 内容 |
|---|---|
| SBR_00 | 数据类型与常量（文档块） |
| SBR_01 | 通用 FB 库索引（指向 `FB\` 各文件夹，不内联定义） |
| SBR_io | IO 映射（物理层：`in_xxx := Xn` 输入映像 + `Yn := out_xxx` 输出映像，**纯透传零逻辑**；地址对照唯一真源） |
| SBR_con | 控制逻辑（安全锁存 / 模式管理 / 远程许可 / 阀·辊筒·轴执行控制 / 报警字汇编 / 控制类输出逻辑量） |
| SBR_host | 通讯主站（**一切通讯事务**：入向心跳看门狗 / 出向心跳 / 上位机命令接单与回执 / Modbus RTU 转发） |
| SBR_status | 状态呈现（三色灯蜂鸣 / 指示类输出 / 上位机状态区回写——**一切对外呈现**，上报实际输出而非命令回声） |
| SBR_08 | 主调度（STEP0~3 统一调用顺序） |

主调度扫描顺序：`io → con → host → status`；控制层安全逻辑最先计算，状态呈现最后统一对外（同周期对外状态自洽）；输出映像透传上周期门控结果（纯透传无中间态）；无许可时 con 层强制安全态。心跳看门狗检测属通讯自检归 host 层，con 层仅消费 `con_CommLost`。

**生成器工程分层变体（2026-09-04 v2.0.0 定稿）**：设备模型库生成的工程采用功能分解 `SBR_02 IO → SBR_03 安全 → SBR_04 模式 → SBR_host 通讯主站 → SBR_05 手动 → SBR_06 自动 → SBR_07 轴与输出 → SBR_status 状态呈现`（SBR_08 主调度按此序调用）。分层原则与本节四模块一致：通讯事务（三级看门狗/出向心跳/恢复沿/急停注销）全归 SBR_host，对外回写全归 SBR_status，各块只读写全局变量表互不调用；本表四模块命名为直编工程（如辊筒）形式，两类工程同构等价。

### 报警机制（六环闭环，强制）

1. **报警源检测**：超时 / 断讯 / 外部故障等一切异常源必须有显式检测逻辑；
2. **可复位故障必须锁存**（FB_AlarmLatch：故障源消除 + 复位边沿才清除）；自动恢复类（如断讯）可即时跟随但必须上报；
3. **报警统一汇编**：`con_AlarmWord : DINT` 位编码（R4），每 bit 一类报警，位定义在 con 表与通讯字表注释齐全；**位定义限于 bit0~15**（上位机寄存器 16 位宽度），status 层回写 INT 通讯字须 `TO_INT(con_AlarmWord)` 显式转换（LiteST 禁 DINT→INT 隐式赋值）；
4. **运行许可唯一真源**：`bRunOK := (con_AlarmWord = 0)`——报警字为零才允许运行；
5. **呈现**：status 层三色灯/蜂鸣（优先级 故障 > 自动 > 手动）+ 上位机报警字寄存器回写；
6. **复位**：本地按钮或远程复位命令边沿，且故障源已消除；只置位不参与联锁的报警 = 形同虚设。
...
### FB 交付形式（强制，2026-08-25 全局变更 / 2026-08-26 补充）

- **每个 FB 独立文件夹**：工程下建 `FB\<FB名>\`，内含两个文件：
  - `<FB名>_变量.csv` —— FB 变量声明表（上方），表头采用**第五张桌面基准** `汇川规范FB变量表.csv`：
    `序号,类别,名称,数据类型,隐藏初始值,初始值,掉电保持,注释,`
  - `<FB名>.st` —— FUNCTION_BLOCK 程序体（下方），与变量表逐行对应
- **类别仅 IN / OUT / VAR**；BOOL 初始值用 `OFF/ON` 字面量；掉电保持填 `保持/不保持`
- **FB 内部库类型实例**（TRIG.R_TRIG、TON 等）作为 **VAR 行**声明在本表：
  数据类型填库类型全名，初始值用复合形式 `"(CLK:=OFF, M:=OFF, Q:=OFF)"`，
  成员行紧随其后（序号/类别留空，名称=`实例.成员`，初始值逐成员填写）
- **禁止在 FB 的 .st 中声明变量**（2026-08-26）：`VAR_INPUT/VAR_OUTPUT/VAR_IN_OUT/VAR...END_VAR`
  块一律不写——所有变量（含内部库实例）只出现在 `_变量.csv`；.st 仅含程序逻辑体。
  审查拦截：ci_check 对 `FB\*\*.st` 出现 VAR 声明关键字即 ERROR
- **SBR_01 不再内联 FB 定义文本**，改为索引说明块（指向 `FB\` 各文件夹）
- 原 `FbTable_*.csv` 形式废弃，由各 FB 文件夹的变量表取代
...
5. **常量化**：加减速系数、超时时间、滤波时间、速度档位、轴状态字一律用 `CONST`，禁止硬编码魔法数字。
   **定时器直调（2026-08-26 强制）**：定时器一律 `TONR(IN:=, PT:=, R:=, Q=>, ET=>)` 直接调用，
   **禁止命名实例**（不得声明 ton_xxx : TON，功能块实例表不得出现 TON 行）；
   Q/ET/R 允许省略。周期节拍用 IN 取反自翻转；看门狗/悬挂超时用单点调用+条件组合
   （多处分支共享同一计时的 必须合并为单一 TONR 调用点）。
...
13. **显式安全切断**：无许可时输出必须显式复位（如辊筒 `A=0 B=0 Dir=0`、步进路径 `=0`、伺服 `Execute=0`），不依赖 `ELSE` 分支的隐式默认。
    **急停命令注销（强制）**：`con_EstopActive`（急停按下或锁存活跃）为真时，必须**主动清零全部命令记忆**（`bRollerRun / bRollerRev / con_CylCmd / bCylPos / con_SpeedTarget` 等）并拒绝一切入向命令字，使急停达到"停机且释放复位后旧命令不自恢复"的目的；不能仅靠 `bRemoteEn` 门控输出的隐式副作用（速度命令 `con_SpeedTarget` 原多为无条件赋值，易被漏清）。该注销块置于通讯层命令处理之前，控制层置 `con_EstopActive` 信号。
14. **电机输出互锁时序**：方向与使能输出都经过许可门控后再刷新到 Y 点。
15. **无线命令防重放（2026-09-04 定稿）**：无线上位机链路存在重发/乱序——每条命令须携带递增序号（`host_Send_CmdSeq`），PLC 记忆最后受理序号（`con_LastCmdSeq`），同序号不二次触发（host_driven 在命令握手 Cmd 门控，internal_seq 在 Start/Home 门控）；**停止/急停类命令永不设防重放与降级门控**；通讯恢复（`con_CommLost` 下降沿）时序号记忆清零，防上位机重启序号回绕导致命令被永久拒绝。
16. **命令回执状态字（2026-09-04 定稿）**：每命令通道提供 `host_Rcv_*_CmdState`（常量 `CMDST_*`：0空闲/2执行中/3完成/4超时，超时保持至下条命令）；上位机在收到回执前**不得认为命令已生效**，禁止以"发送成功"作为执行依据；上位机协议相应为"写参数 → 写启动+新序号 → 轮询回执字确认"。
17. **超时分层（2026-09-04 定稿）**：通讯域超时（命令握手 `TIMEOUT_CMD_HANDSHAKE`、看门狗）与设备域超时（定位/动作 `TIMEOUT_AXIS_MOVE`）**分开定值**——通讯域含无线网络最坏延迟与抖动，须 ≥2× 设备域；设备域为纯有线确定域，按"最远行程/最低速度理论时间 ×1.5"校核，常量注释须标明所属域。
18. **无线看门狗三级降级（2026-09-04 定稿）**：出向心跳独立周期自增（`HEART_PERIOD`，**禁止回显入向心跳**，否则看门狗失效）；入向心跳静默分级——**降级**（`WATCHDOG_DEGRADED`）：置 `con_CommDegraded`，冻结新命令受理，已锁存动作继续完成（容忍漫游/抖动）；**失联**（`WATCHDOG_TIMEOUT`）：`con_CommLost` → `RunOK=0` → 显式安全切断。细则9 的心跳分离为本条基础。
