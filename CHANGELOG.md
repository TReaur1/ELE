...
- **报警汉字显示优化（索引字+对照表，用户选定路线B，生成器+辊筒模板双体系）**：
  - **索引逻辑**：SBR_status 新增报警索引段——扫描 `con_AlarmWord` 取活跃报警编号（活跃 bit 位序+1，多报警同发编号小者优先）写 `con_AlarmIndex`（0=无报警）；生成器侧数据驱动渲染（spec 定义了 ERRID_* 才生成对应判定）。
  - **文本真源**：新增《报警对照表.csv》（GBK，编号/位/标识符/汉字文本/类型/恢复条件）——生成器体系 bit0 急停~bit6 超时 7 行；辊筒模板体系急停/气缸超时/断讯/降级 4 行。HMI 组态以表为准。
  - **配套**：con 表 `con_AlarmIndex`(D2028)；辊筒 const 表 `ALMIDX_ESTOP/CYL/COMM/DEGRADED` 编号常量；生成器 spec 加 `ERRIDX_*` 三常量；辊筒模板 00_说明 新增"报警汉字显示 HMI 组态指引"节（文本列表绑定 con_AlarmIndex + 报警历史按位映射 + 新增报警位四步法）。
  - **验证**：三工程 generate 全过；golden ALL PASS；ci_check 0 错误。
...
- **新增 `工程模板_辊筒线/`：新架构（四模块 + 无线适应细则15~18）下的可复制工程模板**：
  - 母本：`辊筒上位机工程`（四模块定稿参照实现），整目录泛化复制 + 无线四机制移植。
  - SBR_host：单级断讯看门狗升级为**三级**（`COMM_DEGRADED_TIMEOUT` 降级冻结新命令 / `COMM_HEART_TIMEOUT` 失联安全停）；新增**防重放序号门控**（`host_Send_CmdSeq` + `con_LastCmdSeq`，受理即记忆，停止/复位类永不拦截）；新增**通讯恢复沿**（r_CommLostF F_TRIG）清序号记忆防上位机重启序号回绕死锁；速度缓存增加降级冻结。
  - SBR_con：报警字新增 **bit3 通讯降级**（2#1000 随动上报不停机）；辊筒速度档常量化（`ROLLER_SPEED_LEVEL/LOW/MID/HIGH`，消除母本 3 处魔法数字警告）。
  - SBR_status：新增 `host_Rcv_CommState` 三态上报（0正常/1降级/2断讯）。
  - 表：const +5 行（含超时分层注释）、host +2 寄存器（D14/D29）、con +2/-2（新增降级与序号记忆，删除内联化的 bHeartChanged/bHeartTiming）、inst +1 实例（F_TRIG 含成员行）。
  - `00_说明.md`：复制步骤 8 条、上位机侧配套协议、编码红线、与母本差异清单。
  - 验证：ci_check 错误 0（模板部分零警告）；新变量 ST↔表配对自查通过。

## v1.9.0 — 2026-09-04

- **无线适应四机制固化为强制规范（v1.8.0 范例经用户评测通过，复盘闭环落盘）**：
  - AGENTS.md 第五节新增细则 **15 无线命令防重放**（host_Send_CmdSeq + con_LastCmdSeq，停止/急停永不拦截，通讯恢复清序号记忆）、**16 命令回执状态字**（host_Rcv_*_CmdState + CMDST_ 常量，收到回执前不得认为命令生效）、**17 超时分层**（通讯域 ≥2× 设备域，设备域按行程/速度理论时间×1.5 校核）、**18 无线看门狗三级降级**（出向自增禁回显 + WATCHDOG_DEGRADED 降级冻结 / WATCHDOG_TIMEOUT 失联切断）。
  - `汇川PLC_ST编程知识.md` 细则同步扩为 18 条（对外知识保持同源）。
  - 执行侧已由 v1.8.0 生成器实现（三工程产物全带机制）；细则15 的"通讯恢复清序号记忆"句为新增要求，生成器模板待补。
